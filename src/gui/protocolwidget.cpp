    ...
    if (items.size() == 1) {
        ...
        fetchPrivateLinkUrl(data.folder()->accountState()->account(), data.folder()->webDavUrl(), data.folder()->remotePathTrailingSlash() + data.path(),
            parent, [showInWebBrowserAction, parent, pos = menu->actions().size(), menu = QPointer<QMenu>(menu)](const QUrl &url) {
                ...
                if (menu) {
                    auto replacement = new QAction(CommonStrings::showInWebBrowser());
                    connect(replacement, &QAction::triggered, [url, parent] { Utility::openBrowser(url, parent); });
                    menu->insertAction(showInWebBrowserAction, replacement);
                    menu->removeAction(showInWebBrowserAction);
                }
            });
