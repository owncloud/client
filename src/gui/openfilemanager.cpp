...
{
    QProcess p;
    p.start(QStringLiteral("xdg-mime"), QStringList() << QStringLiteral("query")
    ...
    QStringList subdirs;
    subdirs << QStringLiteral("/applications/")
            << QStringLiteral("/applications/kde4/");
...
void showInFileManager(const QString &localPath)
{
    if (Utility::isWindows()) {
        ...
        static QString name;
        if (name.isEmpty()) {
            name = desktopFile.value(QStringLiteral("Desktop Entry/Name[%1]").arg(qApp->property("ui_lang").toString())).toString();
            if (name.isEmpty()) {
