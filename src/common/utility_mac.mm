...
void Utility::setLaunchOnStartup(const QString &appName, const QString &guiName, bool launch)
{
    Q_UNUSED(appName)
    Q_UNUSED(guiName)
        ...
        if ([fileManager fileExistsAtPath:plistFile]) {
            if (launch) {
                ...
                if (!maybePlist) {
                    ...
                    auto result = writeNewPlistFile(plistFile, fullPath, launch);
                    if (!result) {
                ...
                if (programValue == nil) {
                    ...
                    auto result = modifyPlist(plistFile, plist, launch);
                    if (!result) {
                        qCWarning(lcUtility) << result.error();
                    }
