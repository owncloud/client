...
WebFinger::WebFinger(const AccountPtr &acc, QObject *parent)
    : QObject(parent)
    , _account(acc)
{
}

void WebFinger::start(const QString &resourceId)
{
    //    GET /.well-known/webfinger?rel=http://webfinger.owncloud/rel/server-instance&resource=acct:test@owncloud.com HTTP/1.1
    if (OC_ENSURE(_account->url().scheme() == QLatin1String("https"))) {
        QUrlQuery query;
        query.setQueryItems({ { QStringLiteral("resource"), QString::fromUtf8(QUrl::toPercentEncoding(resourceId)) },
            { QStringLiteral("rel"), relId() } });

        QNetworkRequest req;
        req.setUrl(Utility::concatUrlPath(_account->url(), QStringLiteral(".well-known/webfinger"), query));
