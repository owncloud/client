    ...
    : _localModifier(_tempDir.path())
{
    ...
    _syncEngine.reset(new OCC::SyncEngine(account().get(), account()->davUrl(), localPath(), QString(), _journalDb.get()));
...
void FakeFolder::switchToVfs(QSharedPointer<OCC::Vfs> vfs)
{
    ...
    OCC::VfsSetupParams vfsParams(account()->sharedFromThis(), account()->davUrl(), false, &syncEngine());
