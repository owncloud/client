Feature: Syncing files
    ...
    Scenario: Sync all is selected by default
        ...
        Then the file "textfile0.txt" should exist on the file system
    ...
    Scenario: Sync only one folder from the server
        ...
        When user "Alice" uploads file with content "test content" to "simple-folder/uploaded-lorem.txt" in the server
        And user "Alice" uploads file with content "test content" to "large-folder/uploaded-lorem.txt" in the server
        And user "Alice" creates a file "simple-folder/localFile.txt" with the following content inside the sync folder
            """
            test content
            """
        And the user waits for the files to sync
        Then the file "simple-folder/uploaded-lorem.txt" should exist on the file system
        And the file "large-folder/uploaded-lorem.txt" should not exist on the file system
