...
EOF
    ...
    for i in "$4"; do
        echo "- ${CACHE_ENDPOINT}/${CACHE_BUCKET}/$2/$3/guiReportUpload/$4/index.html\n" >> "$1/comments.file"
        ...
        if [[ -f "$1/serverlog.log" ]]; then
            ...
            echo "Server Logs: ${CACHE_ENDPOINT}/${CACHE_BUCKET}/$2/$3/guiReportUpload/$4/serverlog.log" >> "$1/comments.file"
        fi

        # if there is stacktrace file then add to the comment
        if [[ -f "$1/stacktrace.log" ]]; then
            echo "creating comment for stacktrace"
            echo "Stacktrace: ${CACHE_ENDPOINT}/${CACHE_BUCKET}/$2/$3/guiReportUpload/$4/stacktrace.log" >> "$1/comments.file"
        fi
        ...
        if ! [[ $(find "$1/screenshots" -maxdepth 0 -empty) ]]; then
            ...
            for i in "$1"/screenshots/*; do
                echo "- [$(basename "$i")](${CACHE_ENDPOINT}/${CACHE_BUCKET}/$2/$3/guiReportUpload/$4/screenshots/$(basename "$i"))" >> "$1/comments.file"
            done
        fi
