Feature: Sharing
    ...
    Background:
        ...
        And user "Alice" has been created on the server with default attributes and without skeleton files
    ...
    Scenario: sharee tries to create a file and a folder inside a shared folder without write permission
        ...
        And user "Brian" has set up a client with default settings
        When the user waits for folder "Parent" to be synced
        And user "Brian" tries to create a file "Parent/localFile.txt" with the following content inside the sync folder
            """
            test content
            """
        And user "Brian" tries to create a folder "Parent/localFolder" inside the sync folder
        And the user waits for file "Parent/localFile.txt" to be synced
        And the user waits for folder "Parent/localFolder" to be synced
    ...
    Scenario: sharee deletes a file and folder shared by sharer
        ...
        And the user waits for file "Folder" to be synced
    ...
    Scenario: sharee tries to delete shared file and folder without permissions
        ...
        When the user waits for file "textfile.txt" to be synced
        And the user waits for file "Folder" to be synced
        And the user tries to delete the file "textfile.txt"
        And the user tries to delete the folder "Folder"
        And the user waits for the files to sync
