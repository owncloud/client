...
node('CLIENT') {
    ...
    stage 'Qt5'
		...
		mkdir build
		cd build
		cmake -DCMAKE_BUILD_TYPE="Debug" -DUNIT_TESTING=1 -DWITH_TESTING=1 ..
		make -j4
    ...
    stage 'Qt5 - clang'
		...
		mkdir build
		cd build
		cmake -DCMAKE_BUILD_TYPE="Debug" -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DUNIT_TESTING=1 -DWITH_TESTING=1 ..
		make -j4
		ctest -V --output-on-failure'''
