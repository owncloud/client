...
Because this timing strategy is rather fragile without the use of NTP, ownCloud 4.5 introduced a unique number (for each file?) that changes whenever the file changes.
...
Because the file number changes if the file changes, its use is guaranteed to determine if one of the files has changed and, thereby, launching a synchronization process.

NOTE: ownCloud Client release 1.1 and later requires file ID capabilities on the ownCloud server.
...
[source,json]
....
include::example$checksum-algorithm-response.json[]
....
...
If a pattern selected using a checkbox in xref:visualtour.adoc#the-ignored-files-editor[the Ignored Files Editor] (or if a line in the exclude file starts with the character `]` directly followed by the file pattern), files matching the pattern are considered _fleeting meta data_.
...
For more information, please refer to http://pubs.opengroup.org/onlinepubs/009695399/utilities/xcu_chap02.html#tag_02_13_01[The opengroup website].
