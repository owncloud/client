...
function(__addIcon QRC_PATH THEME ICON_NAME)
  ...
  if(EXISTS ${OEM_THEME_DIR}/${icon})
    ...
    if(EXISTS ${OEM_THEME_DIR}/${icon})
      __add_file_to_qrc_file(QRC_PATH ${QRC_PATH} FILE_PATH ${OEM_THEME_DIR}/${icon} ALIAS ${iconAlias})
    else()
...
function(generate_theme TARGET OWNCLOUD_SIDEBAR_ICONS_OUT)
  ...
  set(${OWNCLOUD_SIDEBAR_ICONS_OUT}
      ${OWNCLOUD_SIDEBAR_ICONS}
      PARENT_SCOPE)
endfunction()
...
function(generate_qrc_file)
  ...
  foreach(file ${GENERATE_QRC_FILE_FILES})
    ...
    __add_file_to_qrc_file(QRC_PATH ${GENERATE_QRC_FILE_QRC_PATH} FILE_PATH ${file} ALIAS ${file_alias})
  endforeach()

  __write_qrc_file_footer(${GENERATE_QRC_FILE_QRC_PATH})
endfunction()

# add resources to a target using the Qt resources system parameters: - TARGET: the target to bundle the resources with - PREFIX: virtual "subdirectory" the
# files will be available from - FILES: the files to bundle
function(add_resources_to_target)
  ...
  generate_qrc_file(QRC_PATH ${qrc_path} PREFIX ${ADD_RESOURCES_TO_TARGET_PREFIX} FILES "${ADD_RESOURCES_TO_TARGET_FILES}")
